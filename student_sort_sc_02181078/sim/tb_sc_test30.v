        ...
        for (cycle = 0; cycle < 1000; cycle = cycle + 1) begin
            ...
            if (seen_jal_store && seen_return_store) begin
                if (U_DM.dmem[0] !== 32'h0000000c) begin
                    $display("SC TEST30 FAIL: mem[0]=%h", U_DM.dmem[0]);
                    $finish;
                end
                if (U_DM.dmem[1] !== 32'h98765001) begin
                    $display("SC TEST30 FAIL: mem[4]=%h", U_DM.dmem[1]);
                    $finish;
                end
                if (U_DM.dmem[2] !== 32'h00001236) begin
                    $display("SC TEST30 FAIL: mem[8]=%h", U_DM.dmem[2]);
                    $finish;
                end
                $display("SC TEST30 PASS: mem0=%h mem4=%h mem8=%h first_mem12=00000561 first_mem16=0000055c",
                         U_DM.dmem[0], U_DM.dmem[1], U_DM.dmem[2]);
                $finish;
            end
        end

        $display("SC TEST30 FAIL: timeout seen_jal_store=%b seen_return_store=%b mem0=%h mem12=%h mem16=%h",
